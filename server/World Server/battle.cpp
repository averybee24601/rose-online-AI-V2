...
// Simplified damage calculation function
long int CCharacter::CalculateDamage(CCharacter* attacker, CCharacter* defender, CSkills* skill) {
    // Basic damage calculation (to be refined)
    long int base_damage = attacker->Stats->Attack_Power;
    if (skill) {
        base_damage += skill->atkpower;
    }

    long int defense = defender->Stats->Defense;
    if (attacker->Stats->MagicAttack == 1) {
        defense = defender->Stats->Magic_Defense;
    }

    long int damage = base_damage - defense;
    if (damage < 0) {
        damage = 0;
    }

    // Add randomness to damage
    damage = GServer->RandNumber(damage * 0.9, damage * 1.1);

    return damage;
}

// Simplified normal attack damage calculation function
long int CCharacter::CalculateNormalAttackDamage(CCharacter* attacker, CCharacter* defender) {
    // Basic damage calculation (to be refined)
    long int base_damage = attacker->Stats->Attack_Power;
...
void CCharacter::NormalAttack( CCharacter* Enemy )
{
    ...
    long int hitpower = CalculateNormalAttackDamage(this, Enemy);
...
bool CCharacter::SkillAttack( CCharacter* Enemy, CSkills* skill )
{
    ...
    long int hitpower = CalculateDamage(this, Enemy, skill);
