		...
		for(dword i = 0; i < BlockCount; i++)
	    {
            ...
            if(lma_export)
            {
                ...
                switch(i)
                {
                    ...
                    {
                        extra_rl=false;
                        snprintf(titlegen, sizeof(titlegen), "On Spawn");
                    }
                    break;
                    case 1:
                    {
                        snprintf(titlegen, sizeof(titlegen), "Idle");
                    }
                    break;
                    case 2:
                    {
                        snprintf(titlegen, sizeof(titlegen), "In Combat");
                    }
                    break;
                    case 3:
                    {
                        snprintf(titlegen, sizeof(titlegen), "Is attacked");
                    }
                    break;
                    case 4:
                    {
                        snprintf(titlegen, sizeof(titlegen), "Killed enemy");
                    }
                    break;
                    case 5:
                    {
                        snprintf(titlegen, sizeof(titlegen), "Dead");
                    }
                    break;
                    default:
                    {
                        snprintf(titlegen, sizeof(titlegen), "??Unknown??");
                    }
                    break;
...
void CWorldServer::ExportAipData(byte* dataorg,int size,int opcode)
{
    ...
    if(opcode==13)
    {
        ...
        {
            snprintf ( buffer, sizeof(buffer), "enemy");
        }
        else if(data->btCheckTarget == 0) //check self
        {
             snprintf ( buffer, sizeof(buffer), "self");
        }

        if (data->btStatusType == 1) // check for buffs
        {
            snprintf ( buffer1, sizeof(buffer1), "buff");
        }
        else  // check for debuffs
        {
            snprintf ( buffer1, sizeof(buffer1), "debuff");
        }

        if (data->btHave == 1) //True if HAS buffs
        {
           snprintf ( buffer2, sizeof(buffer2), "needs buff");
        }
        else // true if NOT HAS buffs
        {
           snprintf ( buffer2, sizeof(buffer2), "NO buffs needed");
        }
...
void CWorldServer::ExportAipDataA(byte* dataorg,int size,int opcode)
{
    ...
    if(opcode==6)
    {
        ...
        if(data->cMoreLess == 0)
        {
            snprintf(buffer, sizeof(buffer), "biggest");
        }
        else
        {
            snprintf(buffer, sizeof(buffer), "lowest");
        }

        switch(data->cAbType)
            {
            case 0: //level
                snprintf(buffer1, sizeof(buffer1), "level");
            break;
            case 1: //Attack power
                snprintf(buffer1, sizeof(buffer1), "attack power");
            break;
            case 2: //defense
                snprintf(buffer1, sizeof(buffer1), "defense");
            break;
            case 3: //Magic Def
                snprintf(buffer1, sizeof(buffer1), "MDEF");
            break;
            case 4: // HP
                snprintf(buffer1, sizeof(buffer1), "HP");
            break;
            case 5: // Charm
                snprintf(buffer1, sizeof(buffer1), "charm");
            break;
        }
    ...
    if(opcode==35)
    {
        ...
        switch(data->btOp)
        {
            ...
            case 9:
                ...
                break;
            default:
                snprintf(buffer, sizeof(buffer), "?(%i)?",data->btOp);
                break;
        }
