    ...
    else if (strcmp(command, "go")==0) // Use SQL by Likol
    {
        ...
        if(result)
        {
            if(mysql_num_rows(result)==1)
            {
                ...
                else if (thisclient->Stats->Level>atoi(row[5]))
                {
                    SendPM(thisclient, "You need to be between Level %i and %i to visit %s !",atoi(row[0]),atoi(row[5]),row[4]);
                }
                else if ( thisclient->Stats->HP < (thisclient->Stats->MaxHP / 2) || thisclient->Stats->HP < 1 || thisclient->Session->inGame == false )
                {
...
/* Original code not needing database
...
/*NOT USED        else if (loc == 13) // Union Wars - Attack
        {
...
*/
/*NOT USED        else if (loc == 14) // Union Wars - Defense
        {
             if (thisclient->Stats->Level<100)
             {
                 SendPM(thisclient, "You need to be a least Level 100 to visit Union Wars!");
...
//        if (i > 0 && i%10 == 0) {
   ...
   else if(strcmp(command, "listquest")==0)
    {
        ...
        for(int i=0;i<10;i++)
            {
              if (thisclient->quest.quests[i].QuestID == 0) continue;
                SendPM( thisclient, "Quest[%i]: %i[%i] %i[%i] %i[%i] %i[%i] %i[%i] %i[%i]" , thisclient->quest.quests[i].QuestID, thisclient->quest.quests[i].Items[0].itemnum, thisclient->quest.quests[i].Items[0].count, thisclient->quest.quests[i].Items[1].itemnum, thisclient->quest.quests[i].Items[1].count, thisclient->quest.quests[i].Items[2].itemnum,thisclient->quest.quests[i].Items[2].count,thisclient->quest.quests[i].Items[3].itemnum,thisclient->quest.quests[i].Items[3].count,thisclient->quest.quests[i].Items[4].itemnum,thisclient->quest.quests[i].Items[4].count);
            }
    }
...
//         }
    ...
    }else if (strcmp(command, "ticket")==0) // Submit ticket
    {
        ...
        {
            pthread_mutex_lock( &DB->SQLMutex );
            DB->QExecute("INSERT INTO tickets (char_name,date,message) VALUES('%s','%s','%s')",thisclient->CharInfo->charname,_strdate(date), ticket.c_str());
            ...
            if(sqlticketresult)
            {
                ...
                mysql_free_result(sqlticketresult);
            }
            pthread_mutex_unlock( &DB->SQLMutex );
...
bool CWorldServer::pakGMGMSkills(CPlayer* thisclient, char* name)
{
    ...
    if(result)
    {
        ...
        else
        {
            ...
            else if (index+nb_gm_skills>=120)
            {
