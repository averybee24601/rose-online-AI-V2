...
// Simplified attack power calculation function
unsigned int CPlayer::CalculateAttackPower() {
    UINT attack = 0;
    UINT weaponatk = 0;
    ...
    // Add attack power from other equipment
    for (UINT i = 1; i < 12; i++) {
        if (items[i].count > 0) {
            if (items[i].itemtype > 9) {
                continue;
...
// Wrapper for CalculateAttackPower
unsigned int CPlayer::GetAttackPower2() {
...
//        aspeed -= (aspeed/100)*(Stats->Reborn*5);
...
unsigned int CPlayer::GetMoveSpeed( )
{
    ...
    //reborn
    //for(int i = 0;i<Attr->Reborns;i++)
    //    mspeed*=(UINT)1.1447;
//    mspeed -= (mspeed/100)*(Stats->Reborn*5);
    //if(Battle->contatk == true) mspeed = 550;
    return mspeed;
}
...
void CPlayer::SetStats( )
{
    ...
    Stats->Attack_Power = GetAttackPower2( );
