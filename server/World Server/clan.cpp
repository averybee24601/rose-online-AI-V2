...
enum ClanAction {
    CLAN_ACTION_UPDATE_MEMBER = 0xf0,
    CLAN_ACTION_NEW_MEMBER = 0xfa,
    CLAN_ACTION_KICK_MEMBER = 0xfb,
    CLAN_ACTION_CHANGE_RANK = 0xfc,
    CLAN_ACTION_DISBAND = 0xfd,
    CLAN_ACTION_LEAVE = 0xfe,
    CLAN_ACTION_UPDATE_MARK = 0xff
};
...
bool CWorldServer::pakClanManager ( CPlayer* thisclient, CPacket* P )
{
    ...
    switch(action)
    {
        case CLAN_ACTION_UPDATE_MEMBER:
        {
    	    ...
    	    strncpy(otherclient->Clan->clanname,row[2], sizeof(otherclient->Clan->clanname) - 1);
            otherclient->Clan->clanname[sizeof(otherclient->Clan->clanname) - 1] = '\0';
        ...
        case CLAN_ACTION_NEW_MEMBER://new member added
        {
        	MYSQL_ROW row;
            /*int charid = GETWORD((*P),1);
            int clanid = GETWORD((*P),3);*/
        ...
        break;
        case CLAN_ACTION_KICK_MEMBER://Member Kicked
        {
           char nick[30];
           ...
           if(otherclient!=NULL)
           {
	            ...
	            strncpy(otherclient->Clan->clanname,"", sizeof(otherclient->Clan->clanname) - 1);
                otherclient->Clan->clanname[sizeof(otherclient->Clan->clanname) - 1] = '\0';
                GServer->DB->QExecute("UPDATE characters SET clanid=0,clanrank=0 WHERE id=%i", otherclient->CharInfo->charid);
                BEGINPACKET( pak, 0x7e0 );
                ADDBYTE    ( pak, 0x35 );
        ...
        break;
        case CLAN_ACTION_CHANGE_RANK://member change rank
        {
           char nick[30];
        ...
        break;
        case CLAN_ACTION_DISBAND://disorg
        {
        ...
        break;
        case CLAN_ACTION_LEAVE://Member Leave
        {
           char nick[17];
           memcpy( nick, &P->Buffer[1], P->Size );
           CPlayer* otherclient = GetClientByCharName(nick);
        ...
        break;
        case CLAN_ACTION_UPDATE_MARK: // update clan mark
        {
            unsigned int clanid = GETWORD((*P),1);
...
bool CWorldServer::pakCreateClan ( CPlayer* thisclient, CPacket* P )
{
	...
	strncpy(lma_slogan,slogan, sizeof(lma_slogan) - 1);
	lma_slogan[sizeof(lma_slogan) - 1] = '\0';
