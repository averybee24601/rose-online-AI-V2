...
bool CWorldServer::pakDoIdentify( CPlayer *thisclient, CPacket *P )
{
    ...
    thisclient->Session->username[16] = '\\0';
	strncpy( thisclient->CharInfo->charname, row[1],16 );
    thisclient->CharInfo->charname[16] = '\\0';
                ...
                switch (UseList.Index[thisdrop->item.itemnum]->useeffect[0])
                {
                       ...
                       case 92:
                       {
                            if(dropowner->Clan->clanid>0)
                            {
                                ...
                                char buffer[100];
                                snprintf( buffer, sizeof(buffer), "You received %i Clan Points !!", points);
                                GServer->SendPM(thisclient, buffer);
...
bool CWorldServer::pakShout ( CPlayer* thisclient, CPacket* P )
{
    ...
    std::string message = reinterpret_cast<char*>(&P->Buffer[0]);
    std::string sanitized_message = SanitizeString(message);

    if (sanitized_message.length() > 255) { // Prevent buffer overflows
        sanitized_message = sanitized_message.substr(0, 255);
    }
    
	BEGINPACKET(pak, 0x0785);
	ADDSTRING  ( pak, thisclient->CharInfo->charname );
	ADDBYTE    ( pak, 0 );
	ADDSTRING  ( pak, sanitized_message.c_str() );
	ADDBYTE    ( pak, 0 );
...
bool CWorldServer::pakWhisper ( CPlayer* thisclient, CPacket* P )
{
    ...
    std::string message = reinterpret_cast<char*>(&P->Buffer[strlen(msgto) + 1]);
    std::string sanitized_message = SanitizeString(message);
...
if (GetMileageFreeSlotOnline(thisclient) != -1)
{
    switch(Action)
    {
        ...
        {
// 04 01 00 00 00 00 0a 00 00 00 c7 00 00 00 01 00 00 00 36 32 32 38 66 64 61 30 35 31 30 65 63 65 61 39 55 50 6e 76 69 73 73 65 13 61 bb 02 00 00 20 00 00 00 00 00 00 00 00 00 00 00 08 ef d0 02 0b 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 0f 00 00 00 00 00 00 00 00 00 00 61 d1 00 00 00 00 00 00 00 00 00 00 00 00 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 61 00 00
//            BEGINPACKET ( pak, 0x727 );
//            ADDBYTE     ( pak, Action );
//            ADDBYTE     ( pak, 1 );
//            ADDDWORD    ( pak, 0 );
...
bool CWorldServer::pakNPCBuy ( CPlayer* thisclient, CPacket* P )
{
    ...
    if(thisnpc->npctype >= 2001 && thisnpc->npctype <= 2003)
    {
        is_honor = true;
         adding = 1;
    }

    if(thisnpc->npctype >= 2011 && thisnpc->npctype <= 2014)
    {
        is_valor = true;
         adding = 1;
    }

    if(thisnpc->npctype == 1080)
    {
        is_premium = true;
         adding = 1;
    }
	...
	for (int i=0; i<buycount; i++)
    {
        ...
        Log(MSG_WARNING,"sellid %i",tabid,sellid);
...
bool CWorldServer::pakQuestReq( CPlayer* thisclient, CPacket* P )
{
  ...
  switch(action)
  {
      ...
      case 3:
        ...
        // Execute quest trigger; do not use quest slot as TriggerList index
        // Avoid duplicate 0x730 by deferring reply to below.
        success = thisclient->ExecuteQuestTrigger(hash, false);

        // Fallback: some clients may send the trigger hash in a different byte order
        if (success == QUEST_FAILURE)
        {
            dword swapped = ((hash & 0x000000FF) << 24) |
                            ((hash & 0x0000FF00) << 8)  |
                            ((hash & 0x00FF0000) >> 8)  |
                            ((hash & 0xFF000000) >> 24);
            int retry = thisclient->ExecuteQuestTrigger(swapped, false);
            if (retry != QUEST_FAILURE)
            {
                success = retry;
                hash = swapped;
            }
	...
	if (newslot !=0xffff)
    {
        ...
        if(!painting)
        {
			...
			if(thisclient->Position->Map == GServer->Config.doubleExpMap)
			    crafting_exp = crafting_exp*(GServer->Config.doubleExpAmount*GServer->Config.CRAFTING_EXP_RATE);
			else
                crafting_exp = crafting_exp*GServer->Config.CRAFTING_EXP_RATE;
...
bool CWorldServer::pakOpenShop( CPlayer* thisclient, CPacket* P )
{
    ...
    thisclient->Shop->name[sizeof(thisclient->Shop->name) - 1] = '\\0';
...
bool CWorldServer::pakModifiedItem( CPlayer* thisclient, CPacket* P )
{
    ...
    switch(action)
    {
        ...
        case 0x03: //NPC Disassembly
        {
            ...
            snprintf (disassemblyzulie, sizeof(disassemblyzulie), "Your Zulie has decreased by %i", cost);
