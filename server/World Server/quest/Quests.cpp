        ...
        for(dword i = 0; i < BlockCount; i++)
        {
            ...
            for(dword j = 0; j < RecordCount; j++)
            {
                ...
                if(lma_export)
                {
                    ...
                    {
                        snprintf(bconditions, sizeof(bconditions), "0 condition");
                    }
                    else if(trigger->ConditionCount==1)
                    {
                        snprintf(bconditions, sizeof(bconditions), "1 condition");
                    }
                    else
                    {
                        snprintf(bconditions, sizeof(bconditions), "%i conditions",trigger->ConditionCount);
                    }

                    if(trigger->ActionCount==0)
                    {
                        snprintf(bactions, sizeof(bactions), "0 action");
                    }
                    else if(trigger->ActionCount==1)
                    {
                        snprintf(bactions, sizeof(bactions), "1 action");
                    }
                    else
                    {
                        snprintf(bactions, sizeof(bactions), "%i actions",trigger->ActionCount);
                    }
...
void CWorldServer::LoadQuestData()
{
	...
	for(dword i = 1; i < stbQuest->Rows(); i++){
		if(stbQuest->Data(i, 0)){
			strncpy((char*)&buffer, stbQuest->Data(i, 0), sizeof(buffer) - 1);
...
char* CWorldServer::Abilities(int btOp,char* buffer)
{
    ...
    {
        case sReputation:
            snprintf(buffer, 20, "Fame (PLAYERS)");
            break;
        case sSkillPoints:
            snprintf(buffer, 20, "Skill Pts (PLAYERS)");
            break;
        case sPvp:
            snprintf(buffer, 20, "PvP (PLAYERS)");
            break;
        case sGender:
            snprintf(buffer, 20, "gender");
            break;
        case sFace:
            snprintf(buffer, 20, "face");
            break;
        case sHair:
            snprintf(buffer, 20, "hair");
            break;
        case sJob:
            snprintf(buffer, 20, "job");
            break;
        case sUnion:
            snprintf(buffer, 20, "union");
            break;
        case 81:    //LMA: Union Points (no break, it's NOT a mistake)...
            snprintf(buffer, 20, "union points01");
            break;
        case 82:
            snprintf(buffer, 20, "union points02");
            break;
        case 83:
            snprintf(buffer, 20, "union points03");
            break;
        case 84:
            snprintf(buffer, 20, "union points04");
            break;
        case 85:
            snprintf(buffer, 20, "union points05");
            break;
        case sStrength:
            snprintf(buffer, 20, "strength");
            break;
        case sDexterity:
            snprintf(buffer, 20, "dexterity");
            break;
        case sIntelligence:
            snprintf(buffer, 20, "intelligence");
            break;
        case sConcentration:
            snprintf(buffer, 20, "concentration");
            break;
        case sCharm:
            snprintf(buffer, 20, "charm");
            break;
        case sSensibility:
            snprintf(buffer, 20, "sensibility");
            break;
        case sLevel:
            snprintf(buffer, 20, "level");
            break;
        case sStatPoints:
            snprintf(buffer, 20, "stat points");
            break;
        case sMoney:
            snprintf(buffer, 20, "money");
            break;
    case sEXPRate:
            snprintf(buffer, 20, "exp rate");
            break;
        default:
            snprintf(buffer, 20, "?%i?",btOp);
            break;
    }
    ...
    if(opcode==24)
    {
        ...
        switch(data->btOP)
        {
            ...
            case 10:
                ...
                break;
            default:
                snprintf(buffer, 20, "?%i?",data->btOP);
                break;
        }
...
void CWorldServer::ExportQSDDataA(byte* dataorg,int size,int opcode)
{
    ...
    if(opcode==0)
    {
        ...
        switch(data->btOp)
        {
            case 0:
            {
                snprintf(buffer, 100, "delete");
            }
            break;
            case 1:
            {
                snprintf(buffer, 100, "start");
            }
            break;
            case 2:
            {
                snprintf(buffer, 100, "replace (keep items)");
            }
            break;
            case 3:
            {
                snprintf(buffer, 100, "replace (delete items)");
            }
            break;
            case 4:
            {
                snprintf(buffer, 100, "select");
            }
            break;
            default:
            {
                snprintf(buffer, 100, "?%i?",data->btOp);
            }
            break;
        }
