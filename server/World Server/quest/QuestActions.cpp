...
dword GetRewardValue(dword function, dword amount, CPlayer* client, word nDupCNT){
	...
	switch(function){
		case 0:
		{
			dword tempVal = SafeAdd(amount, 0x1E);
            tempVal = SafeAdd(tempVal, (Cha + 0x0a)); // We need to add a way to get all stats totals (Including buff, E, etc) - Drakia
			tempVal = SafeAdd(tempVal, (100 & 0xFFFF));//World Rate
			tempVal = SafeAdd(tempVal, 0x14);//Fame + 0x14
			tempVal = SafeAdd(((tempVal / (client->Stats->Level + 0x46)) / 0x7530), amount);
			return tempVal;
		}
		break;
		case 1:
		{
			dword tempVal = SafeAdd((client->Stats->Level + 3), amount);
			tempVal = SafeAdd(tempVal, ((Cha >> 1) + client->Stats->Level + 0x28));
			tempVal = SafeAdd(tempVal, (100 & 0xFFFF));//World Rate
		...
		case 5:
		{
			dword tempVal = SafeAdd(amount, 0x14);
			tempVal = SafeAdd(tempVal, Cha + 0x0a);
			tempVal = SafeAdd(tempVal, (100 & 0xFFFF));//World Rate
			tempVal = SafeAdd(tempVal, 0x14);//Fame + 0x14
			tempVal = SafeAdd(((tempVal / (client->Stats->Level + 0x46)) / 0x7530), amount);
			return tempVal;
		...
		case 4:
		{
			dword tempVal = SafeAdd(amount, 0x2);
			tempVal = SafeAdd(tempVal, (Cha + client->Stats->Level + 0x28));
			tempVal = SafeAdd(tempVal, 0x28);//Fame + 0x28
			tempVal = SafeAdd(tempVal, (100 & 0xFFFF));//World Rate
		...
		{
			dword tempVal = SafeAdd(amount, 0x14);
			tempVal = SafeAdd(tempVal, Cha + client->Stats->Level);
			tempVal = SafeAdd(tempVal, 0x14);//Fame + 0x14
			tempVal = SafeAdd(tempVal, (100 & 0xFFFF));//World Rate
			return SafeAdd((tempVal / 0x2DC6C0), amount);
		}
		break;
	}
	return amount;
...
QUESTREWD(007)
{
    ...
    if (!IsValidMap(data->iZoneSN) || !IsValidPosition(data->iZoneSN, thispoint.x, thispoint.y)) {
        Log(MSG_HACK, "Invalid teleport attempt by player %s to map %i at (%.2f, %.2f)",
            client->CharInfo->charname, data->iZoneSN, thispoint.x, thispoint.y);
        return QUEST_FAILURE;
    }
