...
void doubleExpMapActivate()
{
    ...
    pthread_mutex_lock( &GServer->DB->SQLMutex );
    ...
    pthread_mutex_unlock( &GServer->DB->SQLMutex );
...
void LoadLottoConfig()
{
    ...
    if(res)
    {
        ...
        GServer->Config.LottoAccess = atoi(row[0]);
        GServer->Config.LottoActive = atoi(row[1]);
        GServer->Config.LottoType = atoi(row[2]);
        GServer->Config.ZulieCost = atoi(row[3]);
        GServer->Config.ZulieLimit = atoi(row[4]);
...
void UpdateZuliePrize(long long int amount)
{
    ...
    if(Prize)
    {
        ...
        GServer->LottoPrize = atoi(Prizerow[0]);
        mysql_free_result(Prize);
    }
    pthread_mutex_unlock( &GServer->DB->SQLMutex );
