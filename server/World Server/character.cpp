...
void CCharacter::UpdatePosition( bool monster_stay_still )
{
    ...
    // Update the character's position based on their current destination and movement speed.
    // This function is called every frame to ensure that the character's position is updated smoothly.
    if(!IsMoving())
    {
        //osprose
        if(IsPlayer())
