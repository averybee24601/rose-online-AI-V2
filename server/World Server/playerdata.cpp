...
bool CPlayer::loaddata( )
{
	...
	MYSQL_RES *result = GServer->DB->QStore("SELECT level,face,hairStyle,sex,classid,zuly,str,dex,_int, con,cha,sen,\
                                         ...
                                         timergraydrop,hairColor,honor,valor,premium,shuffles,spins FROM characters WHERE char_name='%s'", CharInfo->charname);
    ...
    if(Clan->clanid!=0)
    {
    	...
    	else
    	{
    	    ...
    	    Clan->back = atoi(row[3]);
    	    strncpy(Clan->clanname,row[2], sizeof(Clan->clanname) - 1);
            Clan->clanname[sizeof(Clan->clanname) - 1] = '\\0';
    	    Clan->grade = atoi(row[4]);
    	    Clan->clanrank = atoi(row[5]);
        }
    	GServer->DB->QFree( );
    }
...
void CPlayer::savedata( )
{
    ...
    if(Session->userid!=0)
    {
	   ...
	   if(Stats->MP<0)
        ...
        GServer->DB->QExecute("UPDATE characters SET classid=?,level=?,zuly=?,curHp=?,curMp=?,str=?,con=?,\
                              dex=?,_int=?,cha=?,sen=?,exp=?,skillp=?,statp=?, stamina=?,quickbar=?,respawnid=?,\
                              clanid=?,clan_rank=?, townid=?, rewardpoints=?, bonusxp=?, timerxp=?, shoptype=?,\
                              timershop=?, unionid=?, unionfame=?, union01=?, union02=?, union03=?, union04=?, union05=?,\
                               bonusddrop=?, timerddrop=?, bonusstatdrop=?, timerstatdrop=?, bonusgraydrop=?,\
                               timergraydrop=?, hairColor=?,honor=?,valor=?,premium=?,shuffles=?,spins=? WHERE id=?",
                    ...
                    CharInfo->HairColor, CharInfo->Honor, CharInfo->Valor, CharInfo->Premium, CharInfo->LuckyShuffles, CharInfo->LuckySpins, CharInfo->charid);

        //Storage Expansion Save
        GServer->DB->QExecute("UPDATE accounts SET ExpansionTime=?, itemmall_points=? WHERE username = ?",timerexapnsion,CharInfo->Itemmall,Session->username);
