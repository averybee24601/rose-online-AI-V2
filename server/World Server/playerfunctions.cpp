...
enum ItemType {
    ...
    ITEM_TYPE_JEWEL = 7,
    ITEM_TYPE_WEAPON = 8,
    ITEM_TYPE_SUBWEAPON = 9,
    ITEM_TYPE_CONSUMABLE = 10,
    ITEM_TYPE_JEM = 11,
...
UINT CPlayer::GetNewItemSlot( CItem thisitem )
{
    UINT tabsize = 30;
    UINT itemtab = 0;
    switch((ItemType)thisitem.itemtype)
    {
        case ITEM_TYPE_MASK:case ITEM_TYPE_CAP:case ITEM_TYPE_BODY:case ITEM_TYPE_BACK:case ITEM_TYPE_GLOVE:case ITEM_TYPE_SHOE:case ITEM_TYPE_JEWEL:case ITEM_TYPE_WEAPON:case ITEM_TYPE_SUBWEAPON://equip
            itemtab=0;
        break;
        case ITEM_TYPE_CONSUMABLE://consumables
            itemtab=1;
        break;
        case ITEM_TYPE_JEM:case ITEM_TYPE_NATURAL://etc
            itemtab=2;
        break;
        case ITEM_TYPE_PAT://pat
            itemtab=3;
        break;
        case ITEM_TYPE_MOUNT: // Mounts
            itemtab=1;
        break;
        default:
...
{
    Log(MSG_DEBUG,"character buff in %i",Attr->ALLbuff );
    //return Attr->Int;
...
int CPlayer::ExecuteQuestTrigger(dword hash,bool send_packet, UINT index)
{
    ...
    if (trigger == NULL)
    {
        // Try byte-swapped hash as fallback
        ...
        for(unsigned j=index; j < GServer->TriggerList.size(); j++)
        {
            if (GServer->TriggerList.at(j)->TriggerHash == swapped)
            {
                trigger = GServer->TriggerList.at(j);
                hash = swapped; // Update hash for consistency
                if(j+1<GServer->TriggerList.size())
                {
                    my_index=j+1;
                    nexttrigger = GServer->TriggerList.at(j + 1);
        ...
        {
            if(GServer->ServerDebug)
                Log(MSG_WARNING,"EXTP, questid %u (and swapped %u) not found",hash,swapped);
            return QUEST_FAILURE;
        }
        else
        {
            if(GServer->ServerDebug)
                Log(MSG_WARNING,"EXTP, found questid with swapped hash %u (original %u)",swapped,hash);
        }
    }
