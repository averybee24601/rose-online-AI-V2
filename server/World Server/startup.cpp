...
bool CWorldServer::LoadLTB( )
{
    ...
    for (unsigned int k=0;k<MyLTB.record.size();k++)
    {
        ...
        strncpy (tempLTB->NPCname, MyLTB.record.at(k).name.c_str(), sizeof(tempLTB->NPCname) - 1);
        tempLTB->NPCname[sizeof(tempLTB->NPCname) - 1] = '\\0';
        strncpy (tempLTB->LTBstring, MyLTB.record.at(k).sentence.c_str(), sizeof(tempLTB->LTBstring) - 1);
        tempLTB->LTBstring[sizeof(tempLTB->LTBstring) - 1] = '\\0';
    ...
    {
        CLTBstring* tempLTB = new CLTBstring;
        strncpy (tempLTB->NPCname, " ", sizeof(tempLTB->NPCname) - 1);
        tempLTB->NPCname[sizeof(tempLTB->NPCname) - 1] = '\\0';
        strncpy (tempLTB->LTBstring, MyLTBQSD.record.at(k).sentence.c_str(), sizeof(tempLTB->LTBstring) - 1);
        tempLTB->LTBstring[sizeof(tempLTB->LTBstring) - 1] = '\\0';
...
bool CWorldServer::InitDefaultValues()
{
    ...
    for(int j=0;j<9;j++)
    {
        ...
        if (EquipList[j+1].Index == NULL) {
            Log(MSG_ERROR, "Failed to allocate memory for EquipList");
            return false;
        }
    ...
    if (MapList.Index == NULL) {
        Log(MSG_ERROR, "Failed to allocate memory for MapList");
        return false;
    }
    ...
    if (ClassList == NULL) {
        Log(MSG_ERROR, "Failed to allocate memory for ClassList");
        return false;
    }
    ...
    if (nullBreak == NULL) {
        Log(MSG_ERROR, "Failed to allocate memory for nullBreak");
        return false;
    }
    nullBreak->itemnum=0;
    ...
    if (nullequip == NULL) {
        Log(MSG_ERROR, "Failed to allocate memory for nullequip");
        return false;
    }
    nullequip->id = 0;
    ...
    if (nullnatural == NULL) {
        Log(MSG_ERROR, "Failed to allocate memory for nullnatural");
        return false;
    }
    nullnatural->id = 0;
    nullnatural->type = 0;
...
bool CWorldServer::LoadIfoObjects2( )
{
	...
	while( row = mysql_fetch_row(result) )
    {
        ...
        strncpy(thisobject->questtrigger, row[6], sizeof(thisobject->questtrigger) - 1);
        thisobject->questtrigger[sizeof(thisobject->questtrigger) - 1] = '\\0';
        strncpy(thisobject->luaname, row[7], sizeof(thisobject->luaname) - 1);
        thisobject->luaname[sizeof(thisobject->luaname) - 1] = '\\0';
...
bool CWorldServer::LoadCustomEvents( )
{
    ...
    while( row = mysql_fetch_row(result) )
    {
        ...
        strncpy(thisevent->npcname,row[2], sizeof(thisevent->npcname) - 1);
        thisevent->npcname[sizeof(thisevent->npcname) - 1] = '\\0';
        thisevent->npctype = atoi(row[3]);
        thisevent->npcdir = atof(row[4]);
        thisevent->location.x = (float)atof(row[5]);
        thisevent->location.y = (float)atof(row[6]);
        thisevent->map = atoi(row[7]);
        thisevent->radius = atoi(row[8]);
        thisevent->active = atoi(row[9]);
        for(i=1;i<11;i++)
        {
            thisevent->prizecost[i] = atoi(row[9+i]);
        }
        for(i=1;i<11;i++)
        {
            thisevent->prizetype[i] = atoi(row[19+i]);
        }
        for(i=1;i<11;i++)
        {
            thisevent->prizeid[i] = atoi(row[29+i]);
        }
        for(i=1;i<11;i++)
        {
            strncpy(thisevent->prizename[i].prizename ,row[39+i], sizeof(thisevent->prizename[i].prizename) - 1);
            thisevent->prizename[i].prizename[sizeof(thisevent->prizename[i].prizename) - 1] = '\\0';
        }
        strncpy(thisevent->script1,row[50], sizeof(thisevent->script1) - 1);
        thisevent->script1[sizeof(thisevent->script1) - 1] = '\\0';
        strncpy(thisevent->script2,row[51], sizeof(thisevent->script2) - 1);
        thisevent->script2[sizeof(thisevent->script2) - 1] = '\\0';
        strncpy(thisevent->script3,row[52], sizeof(thisevent->script3) - 1);
        thisevent->script3[sizeof(thisevent->script3) - 1] = '\\0';
        strncpy(thisevent->script4,row[53], sizeof(thisevent->script4) - 1);
        thisevent->script4[sizeof(thisevent->script4) - 1] = '\\0';
        strncpy(thisevent->itemname,row[54], sizeof(thisevent->itemname) - 1);
        thisevent->itemname[sizeof(thisevent->itemname) - 1] = '\\0';
        thisevent->collecttype = atoi(row[55]);
        thisevent->collectid = atoi(row[56]);
        thisevent->level = atoi(row[57]);
        thisevent->objvar1 = atoi(row[58]);
        thisevent->objvar2 = atoi(row[59]);
