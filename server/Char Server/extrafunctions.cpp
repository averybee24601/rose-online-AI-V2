...
CCharClient* CCharServer::GetClientByName( const std::string& name )
{
    for(UINT i=0;i<ClientList.size( );i++)
	{
        CCharClient* client = (CCharClient*) ClientList.at( i );
        ...
        if(iequals(client->charname, name))
        {
            return client;
        }
	}
...
// Search the client by char name
CCharClient* CCharServer::GetClientByUserName( const std::string& username )
{
	CCharClient* client;
	for(unsigned int i=0; i<ClientList.size( ); i++)
	{
		client = (CCharClient*)ClientList.at(i);
        if(iequals(client->username, username))
			return client;
	}
	return NULL;
}
...
CClans* CCharServer::GetClanByID( int id )
{
	...
	{
        CClans* thisclan = ClanList.at( i ).get();
		if( thisclan->id == id )
			return thisclan;
	}
	return NULL;
...
// Get channel by id
CChanels* CCharServer::GetChannelByID( UINT id )
{
...
//LMA: Checking avatar name to avoid GM tags and not valid chars.
bool CCharServer::CheckValidName(CCharClient* thisclient, const char* my_name)
{
    //forbidden chars in names.
    const char *str[] = {"\\","'"," ","`","´",",",".","\""};
    int n;
    int nb_car=strlen(my_name);
    if(nb_car>16||nb_car<4)
    {
        Log(MSG_HACK,"%s tried to create a too long avatar %s (%i)",thisclient->charname.c_str(),my_name,nb_car);
        return false;
    }

    sprintf (my_name,"%s",my_name);

    //uppercase time.
   for (int k=0;k<nb_car;k++)
    {
        int c=toupper(my_name[k]);
        my_name[k]=c;
    }

    const char* valid_chars="AZERTYUIOPQSDFGHJKLMWXCVBN0123456789";
    int i = strspn (my_name,valid_chars);

    if(i!=nb_car)
    {
        Log(MSG_HACK,"%s tried to create a not correct avatar name %s",thisclient->charname.c_str(),my_name);
        return false;
    }
    ...
    {
        if(strstr(my_name,str[n])!=NULL)
        {
            Log(MSG_HACK,"%s tried to create an avatar with a reserved extension %s is in %s",thisclient->charname.c_str(),str[n],my_name);
            return false;
        }
