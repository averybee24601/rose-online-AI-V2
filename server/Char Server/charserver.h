...
class CCharClient : public CClientSocket
{
    public:
    	...
    	bool isLoggedIn;
    	bool logout;
    	vector<std::unique_ptr<CFriendList>> FriendList;
};

// This class is responsible for handling all of the character server's logic.
class CCharServer : public CSocketServer
{
    public:
    	CCharServer ( string );
    	~CCharServer( );
    	...
    	CCharClient* GetClientByName( const std::string& name );
    	CCharClient* GetClientByUserName( const std::string& username );
        ...
        // Variables
        string filename;
        vector <std::unique_ptr<CClans>> ClanList;
        vector <std::unique_ptr<CChanels>> ChannelList;
        vector <std::unique_ptr<CItemNewChar>> ItemNewChar;

        //LMA: chatroom list.
        map<DWORD,std::unique_ptr<CChatroom>> chatroom_list;    //LMA: Chatroom list.
