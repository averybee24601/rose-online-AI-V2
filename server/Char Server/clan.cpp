...
{
    Log(MSG_INFO,"[CS]in lma_mask for %s",thisclient->charname.c_str());
    if( thisclient->clanid > 0 )
    {
        ...
        if(thisclan!=NULL)
        {
            ...
            Log(MSG_INFO,"[CS]lma_mask sent for %s",thisclient->charname.c_str());
        }
...
{
    Log(MSG_INFO,"[CS]in SendClanGrade for %s",thisclient->charname.c_str());
    if(thisclient->clanid <= 0 )
        return true;
...
bool CCharServer::ChangeClanStatus (CCharClient* thisclient, CCharClient* otherclient, int channel)
{
        ...
        */
        ...
        if(thisclient)
            Log(MSG_INFO,"[CS] ChangeClanStatus for %s, job %i, level %i, channel %i",thisclient->charname.c_str(),thisclient->job,thisclient->level,channel);
...
bool CCharServer::pakClanManager ( CCharClient* thisclient, CPacket* P )
{
    ...
    switch (action)
    {
        ...
        case 0x02://invite clan
        {
            ...
            if(iequals(nick,thisclient->charname))
            {
                delete []nick;
                return true;
            }
            ...
            if(otherclient!=NULL)
            {
                ...
                {
                    Log(MSG_INFO,"%s tried to invite %s into his clan, but he was already in clanid %i",thisclient->charname.c_str(),otherclient->charname.c_str(),otherclient->clanid);
                    delete []nick;
                    break;
                }
                ...
                ADDBYTE    ( pak, 0x0 );
                otherclient->SendPacket( &pak );
                Log(MSG_INFO,"[CS] Clan invite %s",thisclient->charname.c_str());
            }
            delete []nick;
        }
        break;
        case 0x03://Kick from clan
        {
           ...
           if(thisclan==NULL)
           {
            ...
            for(UINT i=0;i<thisclan->ClanMembers.size( );i++)
            {
                ...
                if(iequals(ClanMember->name,nick))
                {
           ...
           if( otherclient!=NULL )
           {
            ...
            Log(MSG_INFO,"[CS] delete (kick) %s from clan by %s",nick,thisclient->charname.c_str());
            delete []nick;
        }
        break;
        case 0x04://up rank
        {
            ...
            memcpy( nick, &P->Buffer[1], P->Size-7 );
            Log(MSG_INFO,"[CS] clan, %s is trying to up %s",thisclient->charname.c_str(),nick);
        ...
        case 0x05://down rank
        {
            ...
            memcpy( nick, &P->Buffer[1], P->Size-7 );
            Log(MSG_INFO,"[CS] clan, %s is trying to down %s",thisclient->charname.c_str(),nick);
        ...
        case 0x07://Leave Clan
        {
           ...
           SendToClanMembers(thisclient->clanid,&pak);
            ...
            for(UINT i=0;i<thisclan->ClanMembers.size();i++)
            {
                ...
                if(iequals(ClanMember->name,thisclient->charname))
                {
                    thisclan->ClanMembers.erase( thisclan->ClanMembers.begin()+i );
                    delete ClanMember;
                    break;
	        ...
	        cryptPacket( (char*)&pak, NULL );
 	        ...
 	        if(thischannel!=NULL)
        	    send( thischannel->sock, (char*)&pak, pak.Size, 0 );
            Log(MSG_INFO,"[CS] clan, %s leaves clan",thisclient->charname.c_str());
        }
        break;
        case 0x08:
        {
        ...
        case 0x09://Give Master
        {
            ...
            Log(MSG_INFO,"[CS] clan, %s makes %s master",thisclient->charname.c_str(),nick);
        ...
        case 0x0a://disorganize clan
        {
            ...
            thisclient->clanid = 0;
            thisclient->clan_rank = 1;
            Log(MSG_INFO,"[CS] clan, %s disorg clan",thisclient->charname.c_str());
 	        //send( wct
        }
        break;
        case 0x0c://invitation accepted
        {
           ...
           Log(MSG_INFO,"[CS] clan, %s accepted invitation",otherclient->charname.c_str());
        ...
        case 0x0f:// Update Clan window with Level and Job
        {
            ...
            SendToClanMembers(thisclient->clanid,&pak);
            Log(MSG_INFO,"[CS] clan, update clan window for %s, %i, %i, channel %i",thisclient->charname.c_str(),GETWORD ((*P),1),GETWORD ((*P),3),thisclient->channel);
        }
        break;
        case 0xfa://message from worldserver to load the new clan information
        {
           ...
           if(otherclient==NULL)
        	...
        	ClanList.push_back( std::unique_ptr<CClans>(newclan) );
        	DB->QFree( );
        ...
        case 0xff:
             {
                ...
                if(otherclient!=NULL)
                {
                     Log(MSG_INFO,"[CS] Forcing clan info for %u, %s, adding clan points %li",lma_id,otherclient->charname.c_str(),lma_points);
                     //test
	...
	if(thisclan!=NULL || thisclan != 0)
	{
        ...
        for(UINT i=0;i<thisclan->ClanMembers.size( );i++)
    	{
            ...
            if(otherclient!=NULL)
            {
                ...
                Log(MSG_INFO,"[CS] Clan, pakClanMembers online %s, channel %i, level %i, job %i",otherclient->charname.c_str(),otherclient->channel,otherclient->level,otherclient->job);
            }
            else
            {
...
{
    if(thisclient==NULL) return false;
    Log(MSG_INFO,"[CS] ClanLogout %s",thisclient->charname.c_str());
    CClans* thisclan = (CClans*) GetClanByID(thisclient->clanid);
    if(thisclan!=NULL)
    {
        for(UINT i=0;i<thisclan->ClanMembers.size();i++)
...
bool CCharServer::pakUploadCM ( CCharClient* thisclient, CPacket* P )
{
	...
	for(int i=0;i<ChannelList.size();i++)
        send( ChannelList.at(i).get()->sock, (char*)&pak, pak.Size, 0 );
