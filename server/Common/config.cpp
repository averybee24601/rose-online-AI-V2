...
char *ConfigGetString( const char *pcFile, const char *pcName, const char *pcDefault )
{
	...
	if ( !ConfigGetEntry( pcFile, pcName, pcRet ) ) { Log(MSG_WARNING, "Config missing string '%s' in %s. Using default.", pcName, pcFile); return strdup( pcDefault ); }
	if (pcRet==0) { Log(MSG_WARNING, "Config null string '%s' in %s. Using default.", pcName, pcFile); return strdup( pcDefault ); }
	pcRetOld = pcRet;
	if ( *pcRet=='"' )
	{
		++pcRet;
		size_t slen = strlen(pcRet);
		if (slen>0) { pcRet[ slen - 1 ] = 0; }
	}
...
unsigned ConfigGetInt( const char *pcFile, const char *pcName, unsigned uDefault )
{
	char *pcRet;
	unsigned uRet;
	if ( !ConfigGetEntry( pcFile, pcName, pcRet ) ) { Log(MSG_WARNING, "Config missing int '%s' in %s. Using default %u.", pcName, pcFile, uDefault); return uDefault; }
	if (pcRet==0) { Log(MSG_WARNING, "Config null int '%s' in %s. Using default %u.", pcName, pcFile, uDefault); return uDefault; }

	uRet = (unsigned)atoi( pcRet );
	free( pcRet );
