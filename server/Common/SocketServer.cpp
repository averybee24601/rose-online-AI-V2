...
bool CServerSocket::StartServer( )
{
    ...
    Log( MSG_NOTICE, "Tip: open %shealth.txt for a quick snapshot, or type 'health' and press Enter here.", LOG_DIRECTORY );

    // Start interactive console thread (non-blocking) so you can type commands like 'health'
    if(!console_started)
    {
...
{
    // Take a snapshot to avoid holding the lock during network operations
    vector<CClientSocket*> snapshot;
    ...
    {
        CClientSocket* client = snapshot.at( i );
        if(client->isActive)
        {
...
{
    // Snapshot to avoid iterating while the list can change
    vector<CClientSocket*> snapshot;
    pthread_mutex_lock(&clientListMutex);
    snapshot = ClientList;
    pthread_mutex_unlock(&clientListMutex);
...
void CServerSocket::PingClients()
{
    ...
    // Also write a lightweight snapshot for non-technical monitoring
    WriteHealthSnapshot();
    return;
}

void CServerSocket::WriteHealthSnapshot()
{
    ...
    if(timestamp) timestamp[strlen(timestamp)-1] = 0; // strip newline

    // Snapshot file (overwrite)
    FILE *snap = fopen(LOG_DIRECTORY "health.txt", "w+");
    if(snap)
    {
        fprintf(snap, "time=%s\r\nconnected_clients=%u\r\nmaxfd=%d\r\n", timestamp?timestamp:"", clients, maxfd);
        fclose(snap);
    }
    // History file (append)
    FILE *hist = fopen(LOG_DIRECTORY "health_history.log", "a+");
    if(hist)
    {
        fprintf(hist, "%s connected_clients=%u maxfd=%d\r\n", timestamp?timestamp:"", clients, maxfd);
        fclose(hist);
    }
}
...
{
    // Increment under lock for safety
    pthread_mutex_lock(&clientListMutex);
    ConnectedClients++;
    pthread_mutex_unlock(&clientListMutex);
    ...
    if (thisclient==NULL)
    {
        // nothing to close safely if allocation failed
        return;
    }
    thisclient->GS = this;
...
bool CServerSocket::handleCommand( char* cmd )
{
    ...
    if(strcasecmp(cmd, "health")==0)
    {
        UINT clients = 0;
        pthread_mutex_lock(&clientListMutex);
        clients = ConnectedClients;
        pthread_mutex_unlock(&clientListMutex);
        Log( MSG_CONSOLE, "Health: connected_clients=%u, maxfd=%d", clients, maxfd );
        return true;
    }
